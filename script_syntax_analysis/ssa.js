/**
 * Created by michael_du on 8/1/2016. (https://github.com/michaelpdu/auto_tools/tree/dev/src/script_syntax_analysis)
 * Modifyed by Chaoying Liu
 */
            ...
            try {
                ...
                switch (node.type) {
                    ...
                    case esprima.Syntax.ConditionalExpression: {
                        if (node.test.value === true && node.consequent.type === esprima.Syntax.Literal){
                            ret_val = node.consequent.value;
                            case_flag = true;
                        }

                        else if(node.alternate.type === esprima.Syntax.Literal){
                            ret_val = node.alternate.value;
                            case_flag = true;
                        }
                        break;
                    }
                    ...
                    case esprima.Syntax.CallExpression: {
                        ...
                        if (node.callee.type === esprima.Syntax.MemberExpression
                            ...
                            // Number
                            else if (ssa_util.is_numnode(mem_express.object)
                                && mem_express.object.type === esprima.Syntax.Literal) {
                                
                                switch (property) {
                                    case ("toString"): {
                                        if (ssa_util.is_numnode(args[0])) {
                                            ret_val = ssa_util.numnode_to_num(mem_express.object)["toString"](ssa_util.numnode_to_num(args[0]))
                                            case_flag = true;
                                        }
                                        break;
                                    }
                    ...
                    case esprima.Syntax.MemberExpression: {
                        ...
                        //Literal
                        if (node.object.type === esprima.Syntax.Literal 
                        && node.property !== null
                        && node.property.type === esprima.Syntax.Literal) {
                            if (ssa_util.is_string(node.object.value)) {
                                switch (node.property.value) {
                                    case 'length': {
                                        ret_val = node.object.value['length']
                                        case_flag = true;
                                        break;
                                    }
                                }
